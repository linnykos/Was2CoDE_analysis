...
plot_combination <- function(comb) {
  ...
  plot_logfc <- ggplot(res, aes(x = logFC1, y = logFC2, color = Significant)) +
    ...
    coord_cartesian(xlim = c(-max(abs(res$logFC1), abs(res$logFC2)), max(abs(res$logFC1), abs(res$logFC2))), ylim = c(-max(abs(res$logFC1), abs(res$logFC2)), max(abs(res$logFC1), abs(res$logFC2)))) +
