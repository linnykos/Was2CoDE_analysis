...
#########################
res <- nebula_res$summary
names(res)[names(res) == "logFC_CognitiveStatusNo dementia"] <- "logFC_CognitiveStatusNo_dementia"
names(res)[names(res) == "p_CognitiveStatusNo dementia"] <- "p_CognitiveStatusNo_dementia"
...
res <- nebula_res$summary
#define thresholds
pval_vec <- res[,"p_CognitiveStatusNo_dementia"]
pval_adj_vec <- stats::p.adjust(pval_vec, method = "BH")
idx <- which(pval_adj_vec <= 0.05)
...
# Now create the plot
plot0 <- ggplot(res, aes(x = logFC_CognitiveStatusNo_dementia, y = -log10(p_CognitiveStatusNo_dementia), color = GeneType)) +
