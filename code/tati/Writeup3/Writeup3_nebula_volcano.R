...
pval_vec <- res[,"p_Study_DesignationCtrl"]
pval_adj_vec <- stats::p.adjust(pval_vec, method = "BH")
idx <- which(pval_adj_vec <= 0.05)
#define thresholds
pCutoff <- max(pval_vec[idx])
FCcutoff <- quantile(abs(res[,"logFC_Study_DesignationCtrl"]), probs = 0.9)
xlim <- c(-1,1) * quantile(abs(res[,"logFC_Study_DesignationCtrl"]), probs = 0.99)

idx <- which(abs(res[,"logFC_Study_DesignationCtrl"]) <= max(xlim))
ylim <- c(0, max(-log10(pval_vec[idx])))

plot1 <- EnhancedVolcano::EnhancedVolcano(
  res,
  lab = res$gene,
  x = "logFC_Study_DesignationCtrl",
  y = "p_Study_DesignationCtrl",
  pCutoff = pCutoff,
  FCcutoff = FCcutoff,
  xlim = xlim,
  ylim = ylim
...
ggplot2::ggsave(filename = "~/kzlinlab/projects/subject-de/git/subject-de_tati/figures/tati/Writeup3/Writeup3_NEBULA_volcano.png",
...
# Now create the plot
plot0 <- ggplot(res, aes(x = logFC_Study_DesignationCtrl, y = -log10(p_Study_DesignationCtrl), color = GeneType)) +
...
ggplot2::ggsave(filename = "~/kzlinlab/projects/subject-de/git/subject-de_tati/figures/tati/Writeup3/Writeup3_nebula_volcano_colored_by_GeneLists.png",
                plot0, device = "png", width = 7, height = 7, units = "in")

genes_above_threshold <- res %>%
  filter(-log10(p_Study_DesignationCtrl) > -log10(pCutoff)) %>%
  select(gene)

print(genes_above_threshold)

write.csv(genes_above_threshold, "~/kzlinlab/projects/subject-de/out/tati/Writeup3/genes_above_threshold_NEBULA.csv", row.names = FALSE)
write.table(genes_above_threshold, "~/kzlinlab/projects/subject-de/git/subject-de_tati/figures/tati/Writeup3/genes_above_threshold_NEBULA.csv", 
...
# 
# lfc_vec <- res[,"logFC_Study_DesignationCtrl"]
# xlim <- c(-1,1) * quantile(abs(lfc_vec), probs = 0.99)
# 
# pvalue_vec <- res[,"p_Study_DesignationCtrl"]
