...
tmp <- ss_data_norm$Study_Designation  
ss_data_norm$Study_Designation <- factor(gsub(
  pattern = " ",
  replacement = "_",
  x = tmp
))
...
categorical_vars <- c("Sex", "SeqBatch", "Study_Designation")
numerical_vars <- c("PMI", "coded_Age", "genotype_APOE")
...
covariate_df[,"Study_Designation"] <- factor(covariate_df[,"Study_Designation"], levels = c("No_dementia", "Dementia"))
