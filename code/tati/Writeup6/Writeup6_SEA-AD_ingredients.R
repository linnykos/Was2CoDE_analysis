...
all_genes <- unique(c(
  dat_list$eSVD$genes,
  rownames(dat_list$DESeq2$pvalue),
...
combined_df$DESeq2_pval <- safe_match(dat_list$DESeq2$pvalue, rownames(dat_list$DESeq2$pvalue), all_genes)
combined_df$DESeq2_logFC <- safe_match(dat_list$DESeq2$logFC, rownames(dat_list$DESeq2$logFC), all_genes)
