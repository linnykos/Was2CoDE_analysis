...
neb_data <- nebula::scToNeb(obj = seurat_obj,
                            assay = "RNA",
                            id = "donor_id",
                            pred = c("ADNC", "sex", "PMI", "assay", "Ageatdeath", "APOE4status","self_reported_ethnicity"),
                            offset = "nCount_RNA")
...
df <- model.matrix( ~ ADNC + sex + PMI + assay + Ageatdeath + APOE4status + self_reported_ethnicity,
                    data = neb_data$pred)
start_time <- Sys.time()
